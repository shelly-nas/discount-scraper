...
class DirkClient extends GroceryClient {
    ...
    public async getOriginalPrice(anchorHandle: ElementHandle, originalPriceSelector: string[]): Promise<string> {
        try {            
            ...
            const price = await anchorHandle.$$eval(originalPriceSelector[0], (elements, selector) => {
                return elements.map(element => {
                    const span = element.querySelector(selector);
                    return span ? span.textContent : '';
                }).join('');
            }, originalPriceSelector[1]); // Passing the second selector part as an argument to the page function
            logger.debug(`Original price retrieved: '${price}'.`);
            return price.trim();
        } catch (error) {
            logger.warn(`Warn retrieving original price with selector '${originalPriceSelector.join(' > ')}':`, error);
            return '';
        }
    }
    ...
    public async getDiscountPrice(anchorHandle: ElementHandle, discountPriceSelector: string[]): Promise<string> {
        try {
            ...
            logger.warn(`Warn retrieving discount price with selector '${discountPriceSelector.join(', ')}':`, error);
            return '';
        }
    }
}
